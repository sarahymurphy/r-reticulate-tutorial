...
author: "Sarah Y. Murphy (Sarah.Y.Murphy@wsu.edu)"
email: "Sarah.Y.Murphy@wsu.edu"
date: "4/14/2021"
...
## Reticulate
[Reticulate](https://rstudio.github.io/reticulate/) is a library that allows you to open a python environment within R. You can load Python packages and use them within your R script using a mix of Python and R syntax. All data types will be converted to their equivalent type when being handed off between Python and R.
...
When you install reticulate you are also installing [Miniconda](https://docs.conda.io/en/latest/miniconda.html), a lightweight package manager for Python. This will also create a new Python environment on your machine called `r-reticulate`. You do not need to use this environment but we will use this one for this demonstration. [More information about Python environments can be found here](https://docs.python.org/3/tutorial/venv.html).

## Python Code Blocks in R Markdown

[R Markdown](https://rmarkdown.rstudio.com/) can contain both Python and R code blocks. You can click on the `insert` dropdown list in R Studio and select Python or manually create the code block. Once you have a code block you can use Python with no modifications to Python syntax. Python code blocks will run as Python code when you knit your document. 

```{python}
# JUST PUT A SCREEN SHOT
```

## Working with Python Interactively

We can use Python interactively within the console in R studio. To do this, we can use the `repl_python()` command. This opens a Python prompt.
...
This will open the `r-reticulate` Python environment by default. If you've used Python and have a environment with packages loaded that you'd like to use, you can load that using the following commands.
...
# Alternatively, if you have a named environment you want to use you can load it by name:
use_virtualenv("r-reticulate")

```

We can now do simple python commands. 

```{r, eval=FALSE}
# Open the interactive python environment
repl_python()
...
We have one issue now: we don't have any packages installed. We can do basic Python things, but theres a lot we can't do yet. We will install some important packages using reticulate. 
...
We can now load our newly installed python libraries in R. Open a new R script and import these packages using the `import` command. We set are assigning these to some common nicknames for these packages so they are easier to reference later on.
...
Next, we will create our figure. This is a map of air temperature over the United States for any day in 2013 specified in the plotting command. All of the following are Python commands that are part of the cartopy and matplotlib plotting packages. 

Python users define lists with square brackets. However, the equivalent R type to this is a multi-element vector, so we must define it as such. One example of where this is used is in the first line in the following code block. In Python the proper syntax would be `fig = plt.figure(figsize = [15, 5])`, but when we use this command in R with reticulate we have to use types that R recognizes, so we replace the `=` with `<-`, `.` with `$`, and `[]` with `c()` giving us `fig <- plt$figure(figsize = c(15, 5))`. 
...
I have named this file `Python_PlotAirTemp.py` and have placed it in my current working directory. Once we source this file, we can use the function. Our function is called `PlotAirTemp`.
